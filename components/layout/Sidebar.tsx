...
export function Sidebar({active="Ana Sayfa"}:SidebarProps){
 ...
 return <aside className="border-b border-white/10 bg-[#070811]/88 px-4 py-4 backdrop-blur-2xl lg:min-h-screen lg:w-72 lg:border-b-0 lg:border-r lg:px-5 lg:py-6">
  ...
  <div className="mt-6 hidden lg:block">{groups.map(group=><div key={group.title} className="mb-5 last:mb-0"><p className="mb-1.5 px-3 text-[10px] font-black uppercase tracking-[.2em] text-zinc-600">{group.title}</p><nav className="space-y-1">{group.items.map(item=><NavLink key={item.href} item={item} active={active}/>)}</nav></div>)}</div>
