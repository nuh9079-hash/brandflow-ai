...
export function AnalyticsSummary() {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/analytics/overview?range=30d&platform=all")
  ...
  return (
    <Card className="mb-5 p-5">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[{ label: "Medya", value: data?.metrics.totalMedia }, { label: "Plan", value: data?.metrics.totalScheduledPosts }, { label: "Advisor", value: data?.metrics.advisorReports }, { label: "Ort. skor", value: data?.metrics.averageAdvisorScore }].map((item) => <div key={item.label} className="rounded-lg border border-white/10 bg-white/[0.03] p-3"><p className="text-xs text-zinc-500">{item.label}</p><p className="mt-1 text-xl font-black">{item.value ?? "-"}</p></div>)}
      </div>
    </Card>
  );
}
