...
export async function GET(request: Request) {
  ...
  if (!userId) return redirect(request, "error", "session_missing");
  ...
  try {
    if (!validateInstagramState(state, stateCookie, userId)) throw new Error("invalid_state");
    ...
    const saved = await saveInstagramConnection(userId, token, profile);
    if (!saved.ok) {
      await markInstagramConnectionError(userId, saved.code);
    ...
    await markInstagramConnectionError(userId, safe.code);
