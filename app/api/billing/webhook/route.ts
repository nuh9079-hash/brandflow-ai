...
import { hasProcessedStripeEvent, markStripeEvent, saveStripeCustomer, syncStripeSubscription } from "@/lib/billing/server";
import { requireStripe } from "@/lib/billing/stripe";

export async function POST(request: Request) {
  const signature = request.headers.get("stripe-signature");
  const secret = process.env.STRIPE_WEBHOOK_SECRET;
  if (!secret) return Response.json({ error: "STRIPE_WEBHOOK_SECRET yapılandırılmadı." }, { status: 503 });
  if (!signature) return Response.json({ error: "Stripe imzası eksik." }, { status: 400 });
  ...
  try {
    ...
    if (!(await markStripeEvent(event.id, event.type))) throw new Error("Webhook olayı kaydedilemedi.");
